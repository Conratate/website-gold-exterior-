...

  {
    ...
    price: ({ vehicle, package: pkg, length, scope, condition }) => {
      ...
      if (vehicle === "boat") {
        ...
        const topside = TOPSIDE[condition];
        const hull = HULL[condition];
        if (!area || !topside || !hull) return [0, 0];

        // A full job is simply both halves — derived rather than a third
        // hand-kept pair, so it can't drift out of step with the two above.
        const RATES = {
          topside,
          hull,
          both: [topside[0] + hull[0], topside[1] + hull[1]],
        };
