...
export const SERVICES = [
  {
    ...
    price: ({ surface, size }) => {
      const matrix = {
        driveway: {
          small: [325, 375],   // 2-Car
          medium: [475, 549],  // 4-Car
          large: [699, 799],   // Long / Estate
        },
        sidewalk: {
          small: [600, 700],   // Standard Front
          medium: [650, 750],  // Corner Lot
          large: [950, 1200],  // Full Plaza
        },
        fence: {
          small: [275, 329],   // Short Span
          medium: [399, 459],  // Standard Yard
          large: [495, 565],   // Perimeter
        },
      };
...

  {
    ...
    price: ({ type, sqft }) => {
      ...
      // 15k+ tier uses target market prices directly
      if (sqft === "large") {
        const large = {
          flatwork: [11500, 13000],
          softwash: [13500, 18900],
          garage: [7700, 7700],
          school: [11000, 11000],
        };
        return large[type] || [0, 0];
      }
      const rates = {
        flatwork: [0.75, 0.75],
        softwash: [0.75, 1.05],
      ...
      const area = { small: 3000, medium: 10000 };
      const [low, high] = rates[type];
      const a = area[sqft];
      // 6% discount for every 5,000 sq ft over the first 5,000
      const extraBlocks = Math.floor(Math.max(0, a - 5000) / 5000);
      const discount = Math.min(extraBlocks * 0.06, 0.40);
      return [Math.round(low * a * (1 - discount)), Math.round(high * a * (1 - discount))];
    },
  },
