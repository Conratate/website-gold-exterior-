...

  {
    ...
    questions: [
      ...
      },
      {
        ...
        optionHints: () => ({
          pwc: "Jet ski or personal watercraft",
          under20: "Runabout, skiff or small centre console",
          twenty_25: "Day boat or small cruiser",
          twentyfive_30: "Cruiser or larger centre console",
          thirty_35: "Our maximum size",
        }),
      },
      {
        id: "scope",
        ...
        optionHints: () => ({
          topside: "Deck, topsides and exterior surfaces",
          hull: "Below the waterline — the slow, expensive half",
          both: "Everything above and below the line",
        }),
      },
      {
        id: "condition",
    ...
    price: ({ vehicle, package: pkg, length, scope, condition }) => {
      ...
      if (vehicle === "boat") {
        ...
        //
        // Topside and hull are separate numbers because they're separate
        // surfaces. Below the waterline you're cleaning wetted hull, which
        // scales roughly with length. Above it you're cleaning deck, cabin,
        // superstructure and freeboard — which grows much faster, since a
        // bigger boat is taller and wider, not just longer. Using one figure
        // for both is what made a 35-footer look like a half-day job.
        const TOPSIDE_AREA = {
          pwc: 55,
          under20: 220,
          twenty_25: 330,
        ...
        // Rate per sq ft. Hull work is the expensive half by a wide margin —
        // it's in the water, against growth and oxidation, and it takes the
        // time. Topside is a wash and a polish by comparison. Condition then
        // moves both, since a neglected boat is the real cost driver.
        const TOPSIDE = {
          light:    [1.65, 1.85],
          standard: [1.85, 2.0],
          heavy:    [2.3, 2.7],
        ...
        const topsideArea = TOPSIDE_AREA[length];
        const hullArea = HULL_AREA[length];
        const topsideRate = TOPSIDE[condition];
        const hullRate = HULL[condition];
        if (!topsideArea || !hullArea || !topsideRate || !hullRate) return [0, 0];

        // Round each half to a clean 9-ending figure, with a floor that keeps
        // a small job worth the drive out — then price the full job as the
        // exact sum of the two halves a customer would see quoted singly.
        // Deriving it that way (rather than rounding a raw sum) means the
        // arithmetic always checks out on paper: topside + hull = full, to the
        // dollar, never a rounding hair more.
        const clean = (n) => Math.round(n / 10) * 10 - 1;
        const finish = ([lo, hi]) => [
          Math.max(129, clean(lo)),
          Math.max(149, clean(hi)),
        ];
        const topsidePrice = finish([
          topsideArea * topsideRate[0],
          topsideArea * topsideRate[1],
        ]);
        const hullPrice = finish([hullArea * hullRate[0], hullArea * hullRate[1]]);

        const PRICES = {
          topside: topsidePrice,
          hull: hullPrice,
          both: [topsidePrice[0] + hullPrice[0], topsidePrice[1] + hullPrice[1]],
        };
        return PRICES[scope] || [0, 0];
      }
      return [0, 0];
    },
  },
...
// A price range, written the way a person would write it. Fixed-price work —
// the car packages — has the same figure at both ends, and "$109 – $109" reads
// like a bug rather than a flat rate.
export function formatRange(low, high) {
  if (!(high > 0)) return formatMoney(0);
