...

  {
    ...
    price: ({ vehicle, package: pkg, length, scope, condition }) => {
      ...
      if (vehicle === "boat") {
        ...
        const topsideCost = [topsideArea * topsideRate[0], topsideArea * topsideRate[1]];
        const hullCost = [hullArea * hullRate[0], hullArea * hullRate[1]];

        // A full job is simply both halves added up — derived rather than a
        // third hand-kept figure, so it can't drift out of step with the two
        // above. Each half is priced against its own surface.
        const COSTS = {
          topside: topsideCost,
          hull: hullCost,
          both: [topsideCost[0] + hullCost[0], topsideCost[1] + hullCost[1]],
        };
        const cost = COSTS[scope];
        if (!cost) return [0, 0];
        ...
        return [Math.max(129, clean(cost[0])), Math.max(149, clean(cost[1]))];
      }
      return [0, 0];
    },
  },
