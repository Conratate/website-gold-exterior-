...
//     photos:  {                       // optional; see public/reviews/README.md
//       before: "/reviews/x-before.jpg",
//       after:  "/reviews/x-after.jpg",
...
export function ratingStat(reviews = REVIEWS) {
...
export function aggregateRatingSchema(reviews = REVIEWS) {
  const { count, average } = reviewStats(reviews);
...
// Reviews that carry a full before-and-after pair. These are what the
// Transformations strip shows — the work speaking for itself, next to the
// words of the customer it was done for.
export function transformations(reviews = REVIEWS) {
  return reviews.filter((r) => r.photos && r.photos.before && r.photos.after);
}
